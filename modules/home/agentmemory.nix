_: {
  flake.modules.homeManager.base =
    ...
    let
      ...
      crushProg =
        bin:
        ''.["$schema"] = "https://charm.land/crush.json" | .mcp.agentmemory = {type: "stdio", command: "${bin}", args: []}'';
      opencodeProg =
        bin:
        ''.["$schema"] = "https://opencode.ai/config.json" | .mcp.agentmemory = {type: "local", command: ["${bin}"], enabled: true}'';
