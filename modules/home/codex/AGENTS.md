...
anything beyond a trivial edit, plan first and interview the user about
technical implementation, UX, concerns, and tradeoffs - ask pointed,
structured questions before writing code. Write detailed specs upfront to
reduce ambiguity. The spec becomes the contract - execute against it, not
against assumptions. Strip away all assumptions before touching code.
...
For tasks touching many files, decompose the work explicitly and handle it
in focused batches rather than one sprawling pass - a single context
grinding through 20 files sequentially guarantees decay. Split by concern,
complete and verify one batch, then move to the next. Offload discovery to
search (grep/find) instead of loading whole trees into context, and
checkpoint intermediate state to files so a fresh session - or a
non-interactive run - can pick up cleanly. Keep the working context lean:
pull in only what the current batch actually needs.
...
nonexistent variables), compact and summarize the session proactively.
Treat it like a save point. Do not wait for auto-compaction to fire
unpredictably near the context limit. Summarize the session state into a
`context-log.md` so future sessions can pick up cleanly.
...
Tool results can be silently truncated when they are very large. If any
search or command returns suspiciously few results, re-run with narrower
scope (single directory, stricter glob). State when you suspect truncation
occurred.

### Session Continuity
Always prefer resuming the previous session over starting fresh, to
preserve context, workflow state, and session memory. When exploring two
different approaches, branch into separate sessions so each context is
...
confirm the change applied correctly. In-place edits fail silently when the
target text doesn't match due to stale context. Never batch more than 3
...
- If you run out of context, compact and write the summary to a
  `context-log.md` so we can resume cleanly without a cache penalty.
...
### Batch Changes
When the same edit needs to happen across many files, batch the change and
verify each one in context. Don't hand-apply the same edit twenty times
without checking the result each time.
...
Persistent memory lives in the **agentmemory** MCP database — native memory is
disabled. Use the RIGHT feature for each kind of knowledge, and scope every
entry global vs project:
...
(global, then with the current project), `memory_smart_search` for relevance,
and `memory_facet_query(matchAny="scope:global,project:<current-slug>",
