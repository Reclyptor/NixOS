...
### Forced Verification
A successful `write` or `edit` means bytes hit disk. It does not mean the
code compiles. You are FORBIDDEN from reporting a task as complete until
you have:
...
### Sub-Agent Swarming
For tasks touching >5 independent files, you MUST delegate to parallel
children (5-8 files per child). Each child gets its own full context
window. This is not optional. One agent grinding through 20 files
sequentially guarantees context decay.

Pick the transport by what the child needs to know:
- **`subagent_fork`**: a forked child that already sees your completed
  turns, one-shot. Cheapest for related subtasks - it reuses your request
  prefix, so it needs no standalone briefing.
- **`subagent`**: a spawned child with fresh context, so its prompt must
  stand alone. Continuable in the background - follow up with
  `send_message`, survey children with `list_agents`, stop one with
  `interrupt_agent`.
- **`workflow`**: a JavaScript orchestration script that fans children out
  deterministically. Reach for it when the shape of the fan-out matters
  more than any single child.
- **`ralph`**: one immutable objective handed to a sequence of fresh
  children. For grind-it-down work where every round should start clean.

One task per child for focused execution. Offload research, exploration,
and parallel analysis to children to keep your own context clean. Do NOT
poll a background child mid-run - that pulls its internal tool noise into
your context. Wait for the completion notice.

### Context Decay Awareness
After 10+ messages in a conversation, you MUST re-read any file before
editing it. Do not trust your memory of file contents. Compaction may have
silently destroyed that context. You will edit against stale state and
produce broken output.

### Proactive Compaction
If you notice context degradation (forgetting file structures, referencing
nonexistent variables), run `/compact` proactively - it takes no arguments
and summarizes one balanced older span even below the automatic threshold.
Treat it like a save point. Do not wait for automatic compaction to fire
unpredictably under pressure. Summarize the session state into a
`context-log.md` so future sessions or forked children can pick up cleanly.

### File Read Budget
`read` returns at most 2,000 lines and 51,200 bytes per call, and every
window ends with a footer telling you the next `offset` (1-based). For
files over 500 LOC you MUST page through with `offset`/`limit` until you
reach `(End of file - total <n> lines)`. Never assume you have seen a
complete file from a single read: if the footer says "Use offset=... to
continue," you have not.

### Tool Result Blindness
Tool results are bounded in two places. A result over 50,000 bytes spills
to a file and only its head reaches you - the result names the spill path,
so read it or narrow the call. Separately, any tool result over 8,192
characters is pruned on compaction to its first 4,096 and last 1,024
characters, so the middle of a large result disappears from history later
even though you saw it once. If a search or command returns suspiciously
few results, re-run with narrower scope (single directory, stricter glob).
State when you suspect truncation occurred.

### Session Continuity
Sessions are durable JSONL logs under `$DSH_HOME/sessions`, so resume the
existing session instead of starting a fresh one - history, todo list,
plan state, and goals all replay. `dsh --profile headless "<task>"` is
one-shot by design and prints only its final message: use it for a single
self-contained task, never for work you will need to continue. When
exploring two different approaches, fork a child per approach so each
...
- Do not blindly dump large files into context. `grep` and `glob` are
  ripgrep-backed and are the cheapest way to find your own context - reach
  for them before `read`. Agentic search (finding your own context) beats
...
- Long-running commands belong in a background job. Start it, keep
  working, and collect it with `job_output` (`job_list` to see what is
  running, `job_kill` to stop it) rather than blocking a foreground shell
  call for minutes.
...
confirm the change applied correctly. The observation policy already
refuses an `edit` you have not read (`FS_NOT_OBSERVED`) or one whose file
changed underneath you (`FS_STALE_VERSION`) - treat either as proof your
model of that file is stale. Re-read it and reconsider the edit rather than
replaying it. `edit` also requires a unique `old_string` match unless
`replace_all` is set, so a failed match means your context was wrong, not
that you should widen the pattern until something sticks. Never batch more
...
Your system prompt, your tool catalog, and this file are the reusable
request prefix. Everything after them is append-only, so anything that
rewrites the prefix throws away KV-cache reuse for the rest of the session.

- Do not switch models mid-session. Delegate to a child if a subtask needs
  a different model.
- Do not add, remove, or reconfigure plugins mid-session. The tool catalog
  is held constant even across plan mode for exactly this reason.
- When you need to update context (time, file states), communicate via
  messages, not by mutating configuration.
- If you run out of context, `/compact` and write the summary to a
  `context-log.md` so a fresh session can pick up without paying for the
  whole prefix again.
...
When the same edit needs to happen across many files, fan it out - a
`workflow` script, or one `subagent` per batch - and verify each change in
context. Track the batches with `todo_write`, which replaces the whole list
on every call, so send the entire list every time rather than the delta.
...
trailers, no `🤖 Generated with` footers. The subject line is the entire
message.
...
Persistent memory lives in the **agentmemory** MCP database. Its tools reach
you server-qualified — `memory_lesson_save` is
`mcp__agentmemory__memory_lesson_save`, and the same prefix applies to every
tool named below. The `/remember`, `/recall`, `/recap`, `/handoff`, and
`/forget` skills wrap the common flows. Use the RIGHT feature for each kind
of knowledge, and scope every entry global vs project:
...
deduplicate; never blind-append; prune what's stale.

There are no lifecycle hooks on this harness. Nothing captures the session
behind your back, and nothing injects standing directives at session start
the way it does elsewhere — you are the only writer and the only reader.
Recall deliberately at the start of a task, and persist the high-signal facts
before the session ends, or they are gone.
...
session's own id. Yours is exported into every shell call as
`$DSH_SESSION_ID` — use it rather than inventing a name. Deterministic
naming is the point: later, either the user or another agent can map any
session to its worktree — and any worktree back to the session that owns
it — without guessing.
...
### The Token Is Never in Your Environment — Read It From the File
`modules/home/bash/00-init.nix` exports `$GITHUB_TOKEN` into every
interactive shell from the sops secret at
`~/.config/sops/secrets/bash/github-token`. That export does not reach you.
Every shell call you make is spawned through the subprocess seam, which
strips any variable whose name matches `/KEY|PASSWORD|SECRET|TOKEN/i` before
the child starts — `GITHUB_TOKEN` matches `TOKEN`, so it is always absent.
Do not waste a call checking whether it is set. Read the file:

```bash
export GITHUB_TOKEN="$(cat ~/.config/sops/secrets/bash/github-token)"
```

Each shell call starts a fresh shell, so that export does not survive into
the next one. Put it in the same command as the `git` or `gh` invocation
that needs it.

### `gh` Needs No Flags
The `gh` CLI picks `GITHUB_TOKEN` up on its own and prefers it over the
credentials in `~/.config/gh/hosts.yml`. Export the token and run `gh`
normally, and prefer it for anything API-shaped — pull requests, issues,
releases, repo metadata — because it needs no URL rewriting.
